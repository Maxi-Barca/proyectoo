...
const double signalFrequency = 3000;
...
void loop()
{
  ...
  double fftMagnitudes[samples / 2];  // Solo se usa la mitad de la FFT,, aca voy a guardar el vector

  for (int i = 0; i < samples / 2; i++) {
    fftMagnitudes[i] = vReal[i];   //aca lo guardo
  }
  Serial.println("magnitud guardada:");
  PrintVector(fftMagnitudes, samples/2, SCL_FREQUENCY);   //aca lo printeo
