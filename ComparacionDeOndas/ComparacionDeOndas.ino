...
const double signalFrequency2 = 1010;
const double samplingFrequency2 = 5000;
const uint8_t amplitude2 = 110;
...
void loop()
{
  ...
  double ratio2 = twoPi * signalFrequency2 / samplingFrequency2; // Fraction of a complete cycle stored at each sample (in radians)
  for (uint16_t i = 0; i < samples; i++)
  {
    vReal2[i] = int8_t(amplitude2 * sin(i * ratio2) / 2.0);/* Build data with positive and negative values*/
    //vReal[i] = uint8_t((amplitude * (sin(i * ratio) + 1.0)) / 2.0);/* Build data displaced on the Y axis to include only positive values*/
  ...
  Serial.println(picoFrecuencia, 6);

  double picoFrecuencia2 = FFT2.majorPeak();
  Serial.println(picoFrecuencia2, 6);
